...
class ApiClient {
  ...
  async getProducts(organizationId: string, filters?: { category?: string; isActive?: boolean; search?: string }) {
  ...
  async getBatches(organizationId: string, filters?: { status?: string; productId?: string }) {
    const params: Record<string, string> = { organizationId }
  ...
  async getOrders(organizationId: string, filters?: { status?: string; customerId?: string }) {
    const params: Record<string, string> = { organizationId }
    if (filters?.status) params.status = filters.status
