...
Youâ€™ll need Node.js (v22.12 or higher) and pnpm to compile Tablerâ€™s files. If you donâ€™t have them installed, download and install them from the official websites:
...
Select the affected packages (`@firstcrop/core`, `@firstcrop/preview` or `@firstcrop/docs`), pick the bump type (`patch` for fixes, `minor` for new features) and write a one-sentence description, for example: `Added .btn-ghost variant for buttons.`
