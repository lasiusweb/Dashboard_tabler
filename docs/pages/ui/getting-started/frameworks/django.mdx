...
icon: brand-django
seoDescription: Install `@firstcrop/core`, load CSS and optional JS in Django templates, and render a minimal page layout with a Tabler card.
...
Django templates render server-side HTML, which makes Tabler a drop-in: add Tabler classes to your markup and serve the compiled CSS and JS through Django's static files system â€” no Node build step required at runtime.

You will install `@firstcrop/core`, copy its compiled assets into your static directory, and render a first template with a Tabler card.
...
cp node_modules/@firstcrop/core/dist/css/firstcrop.min.css static/css/
cp node_modules/@firstcrop/core/dist/js/firstcrop.min.js static/js/
```
...
```jinja
{% load static %}
<link rel="stylesheet" href="{% static 'css/firstcrop.min.css' %}" />
```
...
```jinja
{% load static %}
<script src="{% static 'js/firstcrop.min.js' %}"></script>
```
