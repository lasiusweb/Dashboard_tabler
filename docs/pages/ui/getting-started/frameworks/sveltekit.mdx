...
icon: brand-svelte
seoDescription: Install `@firstcrop/core`, import CSS and optional JS in SvelteKit, and render a minimal page layout with a Tabler card.
...
Svelte components compile to plain HTML and Tabler provides the styling on top. The only SvelteKit-specific step is loading `firstcrop.min.js` on the client, since pages render on the server first.

You will install `@firstcrop/core`, import its CSS globally, load the JavaScript from `onMount`, and render a first page with a Tabler card.
...
Install `@firstcrop/core` with your preferred package manager:

<TabsPackage name="@firstcrop/core" />
...
```scss
@import '@firstcrop/core/dist/css/firstcrop.min.css';
```

For full theme customization, import SCSS sources instead:

```scss
@import '@firstcrop/core/scss/firstcrop';
```
...
In SvelteKit, load `firstcrop.min.js` only on the client. `onMount` runs only in the browser, so a dynamic import inside it never executes during SSR:

```html
<script>
  import { onMount } from "svelte";
  onMount(() => {
    import("@firstcrop/core/dist/js/firstcrop.min.js");
  });
</script>
```

SvelteKit-specific note: `firstcrop.min.js` accesses `document`, so importing it during SSR can cause `document is not defined`.
