...
icon: brand-nextjs
seoDescription: Install `@firstcrop/core`, import CSS and JS in Next.js, and render a minimal page layout with a Tabler card.
...
Next.js renders on the server first, which is great for performance but means browser-only code needs care: Tabler CSS can be imported globally, while `firstcrop.min.js` must load on the client. This guide shows an App Router pattern that handles both cleanly.

You will install `@firstcrop/core`, add its CSS to your global stylesheet, mount a small client component for Tabler JavaScript, and render a first page with a Tabler card.
...
Next.js-specific note: `firstcrop.min.js` accesses `document`, so load it only on the client. If you import it during SSR, Next.js throws `document is not defined`.
