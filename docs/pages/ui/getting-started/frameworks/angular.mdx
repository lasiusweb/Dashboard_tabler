...
icon: brand-angular
seoDescription: Install `@firstcrop/core`, register CSS and JS in `angular.json`, and render a minimal Angular page with a Tabler card.
...
This guide uses the Angular CLI. You will install `@firstcrop/core`, register its assets in the build config, and render a first component template with a Tabler card.
...
```json
{
  "styles": ["src/styles.scss", "node_modules/@firstcrop/core/dist/css/firstcrop.min.css"]
}
```
...
```json
{
  "scripts": ["node_modules/@firstcrop/core/dist/js/firstcrop.min.js"]
}
```
