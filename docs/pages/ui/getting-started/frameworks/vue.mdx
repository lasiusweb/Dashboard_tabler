...
icon: brand-vue
seoDescription: Install `@firstcrop/core`, import CSS and JS in Vue, and render a minimal page layout with a Tabler card.
...
Vue single-file components render standard HTML templates, so Tabler classes work directly in your markup while `@firstcrop/core` supplies the styling. JavaScript is only needed for interactive components such as dropdowns, modals, and tooltips.

This guide uses a standard Vite-based Vue setup. You will install `@firstcrop/core`, import its CSS and JS in `src/main.js`, and render a first page with a Tabler card.
