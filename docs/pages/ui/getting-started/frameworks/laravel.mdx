...
icon: brand-laravel
seoDescription: Install `@firstcrop/core`, import CSS and JS with Vite, and render a minimal Laravel view with Tabler styles.
...
Laravel pairs naturally with Tabler: Blade templates render plain server-side HTML, so every Tabler component works out of the box â€” no wrappers or adapters needed. Styling comes from Tabler CSS, and a small JavaScript bundle powers interactive components such as dropdowns and modals.

This guide uses Laravel's default Vite setup. You will install `@firstcrop/core`, wire the CSS and JS into your Vite entry files, and render a first Blade view with a Tabler card.
