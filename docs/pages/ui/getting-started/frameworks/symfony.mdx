...
icon: brand-symfony
seoDescription: Install `@firstcrop/core`, import CSS and JS with Symfony asset tooling, and render a minimal page layout with a Tabler card.
...
Symfony's Twig templates render server-side HTML, so Tabler components work without any adapters â€” you add Tabler classes to your markup and let the asset pipeline deliver the CSS and JS.

This guide uses Webpack Encore. You will install `@firstcrop/core`, import its assets in your Encore entries, and render a first Twig template with a Tabler card.
