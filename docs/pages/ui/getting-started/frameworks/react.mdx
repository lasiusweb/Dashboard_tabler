...
icon: brand-react
seoDescription: Install `@firstcrop/core`, import CSS and JS in React, and render a minimal page layout with a Tabler card.
...
React components render standard HTML, so Tabler slots in as the styling layer: you write JSX with Tabler classes and `@firstcrop/core` provides the CSS. JavaScript is only needed for interactive components such as dropdowns, modals, and tooltips.

This guide uses a standard Vite-based React setup. You will install `@firstcrop/core`, import its CSS and JS in your entry file, and render a first page with a Tabler card.
...
import { Tooltip } from '@firstcrop/core'
