...
icon: brand-astro
seoDescription: Install `@firstcrop/core`, import CSS and client-side JS in Astro, and render a minimal page layout with a Tabler card.
...
You will install `@firstcrop/core`, import its CSS in a base layout, add the client script, and render a first page with a Tabler card.
...
Astro-specific note: a plain `<script>` tag in an Astro component is bundled and runs only in the browser, so importing `firstcrop.min.js` there is safe â€” it never executes during server-side rendering:
